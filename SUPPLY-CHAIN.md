...
`package.json` lists four direct runtime dependencies: `better-sqlite3`,
`dotenv`, `ethers`, and `express`. These are what the indexers and the
read-only API need at run time. `npm ls --omit=dev --depth=0` resolves them to
...
`npm ls --depth=0` lists 15 direct dependencies across both groups. The
package has no `engines` field; CI pins Node 22 in `ci.yml`, but that pin is
not enforced on a local or production install.

The lockfile is `lockfileVersion: 3` and holds 679 package entries. Of those,
678 resolve to `https://registry.npmjs.org/`; the manifest package itself
...
3.1.7 (six host-confusion and SSRF advisories, reached through `ajv`, which has
since left the runtime set and remains only in the Hardhat chain) and `qs`
