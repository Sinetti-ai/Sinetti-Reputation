...
import { aggregateReputation, listSettledDeals, publicCard } from "../src/aggregator";
...
describe("data spine integration", function () {
  ...
  it("indexes four settlement paths end-to-end and produces exact cards", async function () {
    const signers = await ethers.getSigners();
    const token = (await (await ethers.getContractFactory("MockUSDC")).deploy()) as Contract;
    const escrow = await deployEscrow();
    await token.waitForDeployment();
    ...
    const repeatBuyer = aggregateReputation(db, await signers[0].getAddress());
    expect(repeatBuyer).to.deep.include({
      deals_settled: 2,
      outcomes: { release: 2, refund: 0, refund_and_slash: 0 },
      rates: { success_rate: 1, dispute_rate: 0.5, timeout_rate: 0 },
      roles: { as_buyer: 2, as_seller: 0 },
      repeat_counterparty: true
    });
    expect(repeatBuyer?.settled_volume).to.deep.equal({
      amount: (AMOUNT * 2n).toString(), token: await token.getAddress(), human: "200.0"
    });

    const timeoutBuyer = aggregateReputation(db, await signers[6].getAddress());
    expect(timeoutBuyer).to.deep.include({
      deals_settled: 1,
      outcomes: { release: 0, refund: 1, refund_and_slash: 0 },
      rates: { success_rate: 0, dispute_rate: 0, timeout_rate: 1 }
    });

    const slashedSeller = aggregateReputation(db, await signers[11].getAddress());
    expect(slashedSeller).to.deep.include({
      deals_settled: 1,
      outcomes: { release: 0, refund: 0, refund_and_slash: 1 },
      rates: { success_rate: 0, dispute_rate: 1, timeout_rate: 0 },
      bonds: { posted_count: 1, slashed_count: 1, returned_count: 0 }
    });
    expect(publicCard(slashedSeller!)).not.to.have.property("settled_volume");

    const disputeReleaseVerifier = aggregateReputation(db, await signers[5].getAddress());
    expect(disputeReleaseVerifier?.outcomes.release).to.equal(1);
    expect(disputeReleaseVerifier?.rates.dispute_rate).to.equal(1);

    const response = await request(createApp(db)).get(`/agents/${await signers[11].getAddress()}`).expect(200);
    const schema = JSON.parse(readFileSync(resolve(process.cwd(), "schemas/reputation-card.schema.json"), "utf8"));
    const ajv = new Ajv2020({ allErrors: true });
    addFormats(ajv);
    expect(ajv.validate(schema, response.body), JSON.stringify(ajv.errors)).to.equal(true);
    expect(response.body).to.have.all.keys(
      "address", "started_at", "trust_tier", "deals_settled", "outcomes", "rates",
      "bonds", "roles", "repeat_counterparty", "graph", "provenance", "identity_anchor"
    );
    // No account layer, so no contacts and no agent_identity — and volume stays
    // withheld to reduce wallet financial profiling.
    ...
    expect(response.body).not.to.have.any.keys("transactions", "deals", "events", "raw_events");
    db.close();
  });
});
