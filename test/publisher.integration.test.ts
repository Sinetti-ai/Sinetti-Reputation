...
function insertDeal(
  ...
  // decides release from refund. A slash is not inferable from the reason — a
  // deal that never posted a bond settles `ruling_refund` too — so it is
  // anchored on a separate BondSlashed log, which is what the aggregator reads.
...
describe("ERC-8004 reputation publishing", function () {
  ...
  beforeEach(async function () {
    ...
    // Two distinct funded signers, not a hardcoded key: the seller registers
  ...
  describe("key isolation", function () {
    ...
    it("walks the graph it is about to assert an absence over", function () {
      ...
      expect([...graph]).to.include("identityanchor");
    ...
    for (const entry of ["index", "api", "indexer", "web", "feedback", "identity", "registries", "standards"]) {
