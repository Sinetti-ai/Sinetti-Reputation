  ...
  describe("API and pages", function () {
    it("serves the directory, search, the merged card and the name lookup", async function () {
      ...
      try {
        ...
        const page = await request(app).get("/r/hardhat/1").expect(200);
        expect(page.text).to.include("Helper");
        expect(page.text).to.include("Declared by the agent");
        expect(page.text).to.include(`href="${CARD_URL}" rel="noopener noreferrer nofollow"`);
        expect(page.text).to.include("Translate");

        const list = await request(app).get("/r/hardhat").expect(200);
        expect(list.text).to.include("Directory");
        expect(list.text).to.include('href="/r/hardhat/2"');

        // Two agents declare the same name, so the lookup lands on a list rather than guessing.
        const lookup = await request(app).get("/lookup?q=Helper").expect(302);
        expect(lookup.headers.location).to.equal("/search?q=Helper");
        const results = await request(app).get("/search?q=Helper").expect(200);
        expect(results.text).to.include("2 matches");
        // One exact match opens the card.
        files.set("https://other.example/agent.json", JSON.stringify({ name: "Someone else", registrations: [{ agentId: 2 }] }));
        clock += 86_401;
        await readAll();
        const exact = await request(app).get("/search?q=helper").expect(302);
        expect(exact.headers.location).to.equal("/r/hardhat/1");
        await request(app).get("/registries/hardhat/directory?page=0").expect(200);
        await request(app).get("/search?q=nobody").expect(200);
        await request(app).get("/search").expect(404);
        await request(app).get("/lookup?q=").expect(404);
