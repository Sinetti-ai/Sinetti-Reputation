...
import { Resolution, resolveDeal } from "./settlement";
...
/**
 * Default fetcher: Node's global fetch with a hard byte cap on the body. The host
 * is resolved first and every address it resolves to goes through `isPrivateHost`,
 * so a public name pointing at a private address is refused like a literal one.
 * The failure surfaces as `unreachable`. The lookup and the connect are two
 * steps, so a name that changes its answer between them is not caught.
 */
 ...
 * and this service may run beside private infrastructure. Literal check; the
 * default fetcher applies it to every address a name resolves to as well.
 */
...
export function resolveFeedbackUri(uri: string): { url: string } | { data: Uint8Array } | { status: FileStatus } {
  ...
  if (parsed.username || parsed.password || parsed.port) return { status: "scheme_refused" };
 ...
 * example after it changes its declared wallet. The old verdict and evidence go
 * with the status, so a card read before the next pass shows the entry as
 * unchecked rather than as verified on stale evidence. Returns the rows reset.
 */
export function recheckFeedback(db: SinettiDatabase, registry: RegistryConfig, agentId?: string): number {
  const address = getAddress(registry.reputationRegistry);
  const result = agentId
    ? db.prepare("UPDATE erc8004_feedback SET file_status = 'pending', verification = 'unverified', evidence_json = NULL WHERE registry_chain_id = ? AND registry = ? AND agent_id = ?")
      .run(registry.chainId, address, agentId)
    : db.prepare("UPDATE erc8004_feedback SET file_status = 'pending', verification = 'unverified', evidence_json = NULL WHERE registry_chain_id = ? AND registry = ?")
...
export const VERIFIED_PAGE_SIZE = 100;

export interface VerifiedAgentRow { agent_id: string; name: string | null; verified: number; total: number }

/**
 * Agents on a registry with at least one verified entry, with the name each declares, if
 * read, by agent id. Derived, never a sign-up. Paged like the directory, so the route
 * has a ceiling at any registry size.
 */
export function listVerifiedAgents(db: SinettiDatabase, registry: RegistryConfig, page = 1): { page: number; pages: number; agents: VerifiedAgentRow[] } {
  const verified = `SELECT agent_id, ${VERIFIED_COUNT_SQL} AS verified, COUNT(*) AS total
  ...
  const { total } = db.prepare(`SELECT COUNT(*) AS total FROM (${verified})`).get(registry.chainId, reputation) as { total: number };
  const pages = Math.max(1, Math.ceil(total / VERIFIED_PAGE_SIZE));
  const current = Math.min(Math.max(1, Math.floor(page)), pages);
  const agents = db.prepare(`
    SELECT f.agent_id, a.name, f.verified, f.total FROM (${verified}) f
    LEFT JOIN erc8004_agents a ON a.registry_chain_id = ? AND a.registry = ? AND a.agent_id = f.agent_id
    ORDER BY CAST(f.agent_id AS INTEGER) LIMIT ? OFFSET ?
  `).all(registry.chainId, reputation, registry.chainId, getAddress(registry.identityRegistry), VERIFIED_PAGE_SIZE, (current - 1) * VERIFIED_PAGE_SIZE) as VerifiedAgentRow[];
  return { page: current, pages, agents };
}
