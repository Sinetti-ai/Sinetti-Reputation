...
import { Resolution, SettledDeal, listSettledDeals } from "./aggregator";
import { SinettiDatabase } from "./db";
import { settlementFeedbackDocument, settlementFeedbackUri } from "./standards";
import { RegistryConfig, makeProvider, resolveAgentBinding } from "./registries";
 ...
 * agent.
 *
 * `publisherIsOwner` exists because the spec forbids it — "The feedback
 * submitter MUST NOT be the agent owner or an approved operator for agentId" —
 * and the call reverts, after gas, if it is true.
 */
export async function checkAgentId(
  ...
  return {
    ...
    publisherIsOwner: owner === getAddress(publisher)
  };
}
