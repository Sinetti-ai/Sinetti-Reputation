...
async function main(): Promise<void> {
  ...
  if (command === "api") {
    ...
    // network (e.g. a tailscale interface address) — never set it to 0.0.0.0 in production.
    const host = resolveHost(process.env);
    createApp(db).listen(port, host, () => console.log(`Sinetti reputation API listening on http://${host}:${port}`));
    return;
  }
