...
- the settlement indexer for Sinetti Escrow events, and the wallet-card
  aggregation derived from them;
...
- the settlement indexer and wallet cards for the Sepolia v0.1.0 escrow;
...
- Seed the Sepolia registry: register the escrow's seller wallets and publish
  one feedback entry per settled deal through the publisher, so the verified
  path is visible on a public registry.
...
- Add evidence classes beyond settlement: an organisation credential (vLEI), a
  personhood credential (First Person Project), an identity anchor carried in
  the deal. Each fits the same slot, a verifiable document reachable from the
  feedback entry.
